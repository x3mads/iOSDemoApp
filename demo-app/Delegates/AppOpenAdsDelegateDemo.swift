...
final class AppOpenAdsDelegateDemo: AppOpenAdsDelegate {
    ...
    func didLoad(placementId: String, result: LoadResult) {
        ...
        Utils.logger.log("app_open { placement_id: \(placementId) } loaded with network: \(network)")
        Task { @MainActor in
            store.recordLoaded(adType: .appOpen, placementId: placementId, network: network)
        }
    }

    func didPresent(placementId: String) {
        Utils.logger.log("app_open { placement_id: \(placementId) } presented")
        record(placementId: placementId, message: "'\(placementId)' presented.")
    }
    
    func failedToPresent(placementId: String, error: PresentError) {
        Utils.logger.error("app_open { placement_id: \(placementId) } failed to present, with error: \(error.localizedDescription)")
    ...
    func didRecordImpression(placementId: String, data: ImpressionData) {
        Utils.logger.log("app_open { placement_id: \(placementId) } recorded impression with revenue: \(data.revenue)")
